...
namespace Chemix.Utils
{
    ...
    public class SplineInterpolator : MonoBehaviour
    {
        ...
        internal class SplineNode
        {
            ...
            internal UnityEvent onArrive;

            internal SplineNode(Vector3 p, Quaternion q, float t, Vector2 io, UnityEvent e) { Point = p; Rot = q; Time = t; EaseIO = io; onArrive = e; }
            internal SplineNode(SplineNode o) { Point = o.Point; Rot = o.Rot; Time = o.Time; EaseIO = o.EaseIO; onArrive = o.onArrive; }
        }
        ...
        public void AddPoint(Vector3 pos, Quaternion quat, float timeInSeconds, Vector2 easeInOut, UnityEvent onArrive)
        {
            if (mState != "Reset")
                throw new System.Exception("Cannot add points after start");

            mNodes.Add(new SplineNode(pos, quat, timeInSeconds, easeInOut, onArrive));
        }
        ...
        void Update()
        {
            ...
            if (mCurrentTime >= mNodes[mCurrentIdx + 1].Time)
            {
                ...
                {
                    mNodes[mCurrentIdx + 1].onArrive.Invoke();
                    mCurrentIdx++;
                }
                else
                {
